  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-r from-[#12141a] to-[#1a1d24] border border-yellow-500/20 rounded-xl p-6 md:p-8 shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-3 text-xs font-bold font-mono text-slate-300 w-full md:w-auto justify-center md:justify-end flex-wrap">
          {[
            { label: 'USD', key: 'USD' },
            { label: 'EUR', key: 'EUR' },
            { label: 'GBP', key: 'GBP' },
          ].map(({ label, key }) => (
            <div key={key} className="bg-[#0f1115] px-4 py-2 rounded-lg border border-[#1f222a] shadow-inner flex flex-col items-center min-w-[72px]">
              <span className="text-slate-500 text-[9px] uppercase tracking-widest mb-1">{label}</span>
              {cotacoesLoading ? (
          ...
          <button
            ...
            className="bg-[#0f1115] px-3 py-2 rounded-lg border border-[#1f222a] text-slate-600 hover:text-yellow-500 hover:border-yellow-500/40 transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div
          ...
          className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl flex flex-col items-center justify-between cursor-pointer group hover:border-yellow-500/50 transition-all h-72"
        >
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest w-full text-center">
        ...
        <div
          ...
          className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl cursor-pointer group hover:border-emerald-500/50 transition-all flex flex-col justify-between h-72"
        >
        ...
        <div
          ...
          className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl cursor-pointer group hover:border-indigo-500/50 transition-all flex flex-col h-72"
        >
          <div className="w-full flex justify-between items-center mb-6">
            ...
            {routinesData[activeRoutine].timeline
              ...
              .map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center gap-3 bg-[#0f1115] p-3 rounded-lg border border-[#1f222a] group-hover:border-[#2d333b] transition-colors"
                  >
                    <IconComponent className="w-4 h-4 text-indigo-400 opacity-80" />
                    <div className="flex-1 overflow-hidden">
                      <p className="text-xs font-bold text-white truncate">{item.title}</p>
                    </div>
                    <span className="text-[10px] font-mono text-slate-500 bg-[#1a1d24] px-2 py-1 rounded">
                      {item.time}
                    </span>
                  </div>
                );
        ...
        <div
          ...
          className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl cursor-pointer group hover:border-yellow-500/50 transition-all flex flex-col justify-between h-72"
        >
          <div>
        ...
        <div className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl flex flex-col justify-between relative overflow-hidden h-72">
          ...
          <div>
            ...
            {showProvasForm ? (
              <div className="space-y-2 overflow-y-auto max-h-44">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#0a0b0e] border border-[#1f222a] rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-yellow-500"
                  />
                  <input
                    type="text"
                    placeholder="dd/mm"
                    ...
                    className="w-20 bg-[#0a0b0e] border border-[#1f222a] rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-yellow-500"
                  />
                ...
                {(provas || []).map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center justify-between bg-[#0a0b0e] rounded px-2 py-1.5 border border-[#1f222a]">
                      <div>
                        <p className="text-xs font-bold text-white">{p.titulo}</p>
                        <p className="text-[10px] text-slate-500">{p.data} — {dias !== null ? (dias < 0 ? 'Passou' : `${dias}d`) : '—'}</p>
                      </div>
                      <button onClick={() => handleDeleteProva(p.id)} className="text-slate-600 hover:text-rose-500">
              ...
              <>
                <p className="text-sm text-slate-400 font-medium">{proximaProva.titulo}</p>
                <div className="mt-4">
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Contagem Regressiva</p>
                  <div className="flex items-baseline gap-2">
                    <span className="text-5xl font-black text-white">{proximaProva.dias}</span>
                    <span className="text-lg text-emerald-500 font-bold uppercase">Dias</span>
                  </div>
                  <p className="text-[10px] text-slate-600 mt-1">Data: {proximaProva.data}</p>
                </div>
              </>
            ) : (
              <div className="flex flex-col items-center justify-center flex-1 h-32 text-center">
                <p className="text-slate-600 text-xs">Nenhuma prova cadastrada.</p>
        ...
        <div
          onClick={() => setActiveTab('Faculdade (ADM)')}
          className="bg-[#12141a] border border-[#1f222a] rounded-xl p-6 shadow-xl cursor-pointer group hover:border-indigo-500/50 transition-all flex flex-col justify-between items-center text-center h-72"
        >
          <div className="w-full flex justify-between items-center mb-2">
            <h2 className="text-indigo-400 font-bold text-xs uppercase tracking-wider flex items-center gap-2">
