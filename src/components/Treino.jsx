  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-[#12141a] border border-[#1f222a] p-4 rounded-2xl flex items-center gap-6 shadow-xl w-full md:w-auto">
          {isEditingProfile ? (
            <div className="flex items-center gap-4">
              <div>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Peso (kg)</p>
                <input 
                  type="number" step="0.1"
                  value={tempProfile.peso} onChange={e => setTempProfile({ ...tempProfile, peso: e.target.value })}
                  className="w-16 bg-[#0a0b0e] border border-yellow-500/50 rounded-lg px-2 py-1 text-white font-bold text-center focus:outline-none"
                />
              </div>
              <div className="h-8 w-px bg-[#1f222a]"></div>
              <div>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Altura (m)</p>
                <input 
            ...
            <div className="flex items-center gap-6 cursor-pointer group" onClick={handleEditProfile} title="Clique para editar">
              <div className="text-center group-hover:text-yellow-500 transition-colors">
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Peso Atual</p>
                <p className="text-xl font-black text-white">{workoutProfile.peso} kg</p>
              </div>
              <div className="h-8 w-px bg-[#1f222a]"></div>
              <div className="text-center group-hover:text-yellow-500 transition-colors">
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Altura</p>
                <p className="text-xl font-black text-white">{workoutProfile.altura} m</p>
              </div>
            </div>
          )}
        </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
        {workouts.map((day) => (
          <div key={day.id} className="bg-[#12141a] border border-[#1f222a] rounded-2xl flex flex-col h-full overflow-hidden shadow-xl group">
            ...
            <div className={`p-5 bg-gradient-to-br ${day.color} relative overflow-hidden`}>
              ...
              <h3 className="text-xl font-black italic text-white relative z-10 uppercase tracking-tight">{day.day}</h3>
              <p className="text-white/90 text-[11px] font-bold tracking-widest uppercase relative z-10 mt-1">
                {day.title} • {day.focus}
              </p>
              <div className="mt-4 flex items-center gap-1.5 text-white text-[10px] font-black tracking-widest uppercase bg-black/20 w-max px-2.5 py-1.5 rounded-lg">
            ...
            <div className="p-4 flex-1 flex flex-col gap-3">
              {day.exercises.map((ex, exIndex) => (
                <div key={exIndex} className="bg-[#0a0b0e] p-3 rounded-xl border border-[#1f222a] hover:border-slate-700 transition-colors group/ex">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <button 
                      ...
                      className="text-slate-600 hover:text-rose-500 opacity-0 group-hover/ex:opacity-100 transition-opacity ml-2"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      <span className="text-[9px] font-black uppercase tracking-widest bg-[#1f222a] text-slate-300 px-2 py-1 rounded-md" title="Séries x Reps">
                        {ex.sets}x {ex.reps}
                      </span>
                      <span className="text-[9px] font-bold flex items-center gap-1 text-slate-500 uppercase tracking-widest" title="Descanso">
                    ...
                    <div>
                      <input 
                        ...
                        className="w-14 bg-[#1f222a] border border-[#2a2d36] rounded text-right text-xs font-black text-white px-2 py-1 focus:outline-none focus:border-yellow-500 focus:bg-[#0a0b0e] transition-all placeholder:text-slate-600"
                      />
                    </div>
                  </div>
                </div>
              ...
              <button 
                ...
                className="w-full mt-2 py-2 border border-dashed border-[#1f222a] rounded-xl text-[10px] font-bold text-slate-500 uppercase tracking-widest hover:border-yellow-500/50 hover:text-yellow-500 transition-all flex items-center justify-center gap-2"
              >
                + Adicionar Exercício
              </button>
            </div>
      ...
      <section className="bg-[#12141a] border border-[#1f222a] rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-8 shadow-xl relative overflow-hidden">
        {/* Decorator line */}
        ...
        <div className="md:w-1/3">
          ...
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Fundamentos do Treino</p>
        </div>

        <div className="md:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-slate-400 leading-relaxed">
          <div className="bg-[#0a0b0e] p-5 rounded-xl border border-[#1f222a]">
            <h3 className="text-white font-bold uppercase text-[11px] tracking-widest mb-3 flex items-center gap-2">
              <Info className="w-3 h-3 text-blue-500" /> Frequência
            </h3>
            <p className="text-xs">Estudos literários demonstram que estimular o mesmo grupo muscular duas vezes por semana maximiza a síntese proteica (MPS). A divisão <span className="text-white font-bold">Upper/Lower 2x</span> garante estímulo contínuo sem overtraining para ectomorfos.</p>
          </div>
          <div className="bg-[#0a0b0e] p-5 rounded-xl border border-[#1f222a]">
            <h3 className="text-white font-bold uppercase text-[11px] tracking-widest mb-3 flex items-center gap-2">
              <Clock className="w-3 h-3 text-blue-500" /> Descanso & ATP
            </h3>
            <p className="text-xs">O American College of Sports Medicine recomenda <span className="text-white font-bold">2 a 3 mins</span> de descanso em exercícios compostos pesados (Terra, Agachamento). Isso regenera o ATP-CP, essencial para manter a carga alta no dia pesado.</p>
          </div>
        </div>

        {/* Checklist Sidebar */}
        <div className="md:w-1/4 mt-8 md:mt-0 md:border-l md:border-[#1f222a] md:pl-8">
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={idx} className="flex items-start gap-2 group cursor-pointer">
                <div className="w-4 h-4 rounded border border-[#1f222a] flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:border-emerald-500 transition-colors">
                ...
                <span className="text-[11px] text-slate-400 font-medium group-hover:text-slate-200 transition-colors">{item}</span>
              </li>
            ))}
          </ul>
        </div>
